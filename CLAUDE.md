...
- **오버레이는 `history.pushState` + `popstate`로 닫는다.** `#infoModal`·`#recordsView`를 열 때 state를 쌓고, `popstate` 한 곳에서 "모달 먼저, 없으면 기록 화면" 순으로 닫는다. 닫기 버튼도 `history.back()`을 부른다(직접 `hidden=true` 금지 — 히스토리가 어긋난다). 새 오버레이를 추가하면 이 핸들러에 순서를 넣어야 한다.
